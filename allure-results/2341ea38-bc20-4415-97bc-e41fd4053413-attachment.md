# Page snapshot

```yaml
- generic:
  - generic:
    - generic [ref=e2]:
      - generic [ref=e3]:
        - link [ref=e4] [cursor=pointer]:
          - /url: javascript:void(0);
          - text: Skip to Navigation
        - link [ref=e5] [cursor=pointer]:
          - /url: javascript:void(0);
          - text: Skip to Main Content
        - generic [ref=e9]:
          - generic [ref=e13]:
            - button [ref=e17] [cursor=pointer]:
              - img [ref=e21]
              - generic [ref=e24]: Menu
            - generic [ref=e29]:
              - img [ref=e33]
              - generic [ref=e37]: Sandbox (ETQA)
          - button [ref=e46] [cursor=pointer]:
            - img [ref=e48]
            - generic [ref=e51]: Show menu
        - generic [ref=e52]:
          - button [ref=e58]:
            - img [ref=e60]
            - text: Search...
          - navigation [ref=e63]:
            - list [ref=e65]:
              - listitem [ref=e66]:
                - group [ref=e67]:
                  - button [ref=e69] [cursor=pointer]:
                    - img [ref=e74]
                  - button [ref=e78] [cursor=pointer]:
                    - img [ref=e83]
              - listitem [ref=e86]:
                - button [ref=e92] [cursor=pointer]:
                  - img [ref=e97]
              - listitem [ref=e100]:
                - button [ref=e102] [cursor=pointer]:
                  - img [ref=e107]
              - listitem [ref=e110]:
                - button [ref=e113] [cursor=pointer]:
                  - img [ref=e118]
              - listitem [ref=e121]:
                - button [ref=e127] [cursor=pointer]:
                  - img [ref=e132]
              - listitem [ref=e135]:
                - button [ref=e138] [cursor=pointer]:
                  - img [ref=e144]
              - listitem [ref=e148]:
                - button [ref=e151] [cursor=pointer]
      - generic [ref=e156]:
        - generic [ref=e159]:
          - generic [ref=e161]:
            - navigation [ref=e162]:
              - button [ref=e164] [cursor=pointer]:
                - generic [ref=e175]: App Launcher
            - heading [level=1] [ref=e176]:
              - generic [ref=e177]: ITGB TPM
          - navigation [ref=e180]:
            - list [ref=e181]:
              - listitem [ref=e182] [cursor=pointer]:
                - link [ref=e183]:
                  - /url: /lightning/page/home
                  - generic [ref=e184]: Home
              - listitem [ref=e185]:
                - link [ref=e186] [cursor=pointer]:
                  - /url: /lightning/o/Account/home
                  - generic [ref=e187]: Accounts
                - button [ref=e191] [cursor=pointer]:
                  - img [ref=e195]
                  - generic [ref=e198]: Accounts List
              - listitem [ref=e199]:
                - link [ref=e200] [cursor=pointer]:
                  - /url: /lightning/o/Product2/home
                  - generic [ref=e201]: Products
                - button [ref=e205] [cursor=pointer]:
                  - img [ref=e209]
                  - generic [ref=e212]: Products List
              - listitem [ref=e213]:
                - link [ref=e214] [cursor=pointer]:
                  - /url: /lightning/n/cgcloud__Trade_Planning
                  - generic [ref=e215]: Trade Planning
              - listitem [ref=e216]:
                - link [ref=e217] [cursor=pointer]:
                  - /url: /lightning/o/cgcloud__Account_Plan__c/home
                  - generic [ref=e218]: Customer Business Plans
                - button [ref=e222] [cursor=pointer]:
                  - img [ref=e226]
                  - generic [ref=e229]: Customer Business Plans List
              - listitem [ref=e230]:
                - link [ref=e231] [cursor=pointer]:
                  - /url: /lightning/o/cgcloud__Promotion__c/home
                  - generic [ref=e232]: Advanced Promotions
                - button [ref=e236] [cursor=pointer]:
                  - img [ref=e240]
                  - generic [ref=e243]: Advanced Promotions List
              - listitem [ref=e244]:
                - link [ref=e245] [cursor=pointer]:
                  - /url: /lightning/o/cgcloud__Fund__c/home
                  - generic [ref=e246]: Funds
                - button [ref=e250] [cursor=pointer]:
                  - img [ref=e254]
                  - generic [ref=e257]: Funds List
              - listitem [ref=e258]:
                - link [ref=e259] [cursor=pointer]:
                  - /url: /lightning/o/cgcloud__Fund_Transaction_Header__c/home
                  - generic [ref=e260]: Multi-Fund Transactions
                - button [ref=e264] [cursor=pointer]:
                  - img [ref=e268]
                  - generic [ref=e271]: Multi-Fund Transactions List
              - listitem [ref=e272]:
                - button [ref=e274] [cursor=pointer]:
                  - img [ref=e276]
                  - generic [ref=e279]: Edit nav items
        - main [ref=e281]:
          - generic [ref=e291]:
            - generic [ref=e292]:
              - article [ref=e296]:
                - generic [ref=e298]:
                  - img [ref=e303]
                  - heading [level=2] [ref=e307]:
                    - generic [ref=e308]: Switch Sales Org
                - generic [ref=e311]:
                  - generic [ref=e312]:
                    - generic [ref=e313]: Switch to
                    - generic [ref=e322]:
                      - combobox [ref=e323] [cursor=pointer]:
                        - generic [ref=e324]: CG Cloud TREx
                      - generic:
                        - generic:
                          - generic:
                            - img
                  - button [ref=e327] [cursor=pointer]: Switch
              - paragraph [ref=e331]:
                - strong [ref=e332]: Connect with Your Customers
              - article [ref=e334]:
                - heading [level=2] [ref=e338]:
                  - generic [ref=e339]: Sales Person Activity
                - generic [ref=e341]:
                  - generic [ref=e344]:
                    - text: Loading report chart...
                    - generic [ref=e346]:
                      - button [ref=e350] [cursor=pointer]: Enable table equivalent for screen reader users
                      - list [ref=e351]
                  - generic [ref=e354]:
                    - link [ref=e355] [cursor=pointer]:
                      - /url: javascript:void(0);
                      - text: View Report
                    - generic [ref=e357]: As of Yesterday at 3:53 PM
              - article [ref=e360]:
                - generic [ref=e361]:
                  - heading [level=2] [ref=e364]:
                    - generic [ref=e365]: Today’s Tasks
                  - button [ref=e368] [cursor=pointer]:
                    - img [ref=e370]
                    - generic [ref=e373]: Select a view of your tasks
                - generic [ref=e377]:
                  - img [ref=e378]
                  - generic [ref=e379]: Nothing due today. Be a go-getter, and check back soon.
                - button [ref=e381] [cursor=pointer]: View All
              - article [ref=e384]:
                - heading [level=2] [ref=e388]:
                  - generic [ref=e389]: Today's Events
                - generic [ref=e393]:
                  - img [ref=e394]
                  - generic [ref=e395]: Looks like you're free and clear the rest of the day.
                - button [ref=e397] [cursor=pointer]: View Calendar
              - paragraph [ref=e401]:
                - strong [ref=e402]: Manage Your Business & Data
              - list [ref=e405]:
                - listitem [ref=e406]:
                  - generic [ref=e407]:
                    - heading [level=2] [ref=e409]:
                      - button [expanded] [ref=e410] [cursor=pointer]:
                        - img [ref=e412]
                        - generic [ref=e415]: Sales Operations
                    - article [ref=e420]:
                      - heading [level=2] [ref=e424]:
                        - generic [ref=e427]: ㅤ
                      - generic [ref=e432]:
                        - link [ref=e436] [cursor=pointer]:
                          - generic [ref=e438]:
                            - img [ref=e442]
                            - link [ref=e444]:
                              - /url: "#"
                              - text: Products
                        - link [ref=e448] [cursor=pointer]:
                          - generic [ref=e450]:
                            - img [ref=e454]
                            - link [ref=e456]:
                              - /url: "#"
                              - text: Price Books
                        - link [ref=e460] [cursor=pointer]:
                          - generic [ref=e462]:
                            - img [ref=e466]
                            - link [ref=e468]:
                              - /url: "#"
                              - text: Quotes
                        - link [ref=e472] [cursor=pointer]:
                          - generic [ref=e474]:
                            - img [ref=e478]
                            - link [ref=e480]:
                              - /url: "#"
                              - text: Contracts
                        - link [ref=e484] [cursor=pointer]:
                          - generic [ref=e486]:
                            - img [ref=e490]
                            - link [ref=e492]:
                              - /url: "#"
                              - text: Orders
                        - link [ref=e496] [cursor=pointer]:
                          - generic [ref=e498]:
                            - img [ref=e502]
                            - link [ref=e504]:
                              - /url: "#"
                              - text: Forecasts
                - listitem [ref=e505]:
                  - heading [level=2] [ref=e508]:
                    - button [ref=e509] [cursor=pointer]:
                      - img [ref=e511]
                      - generic [ref=e514]: Email
                - listitem [ref=e515]:
                  - heading [level=2] [ref=e518]:
                    - button [ref=e519] [cursor=pointer]:
                      - img [ref=e521]
                      - generic [ref=e524]: Data
            - generic [ref=e525]:
              - paragraph [ref=e529]:
                - strong [ref=e530]: Manage & Close Deals
              - article [ref=e532]:
                - heading [level=2] [ref=e536]:
                  - generic [ref=e537]: Team Pipeline
                - generic [ref=e539]:
                  - generic [ref=e542]:
                    - text: Loading report chart...
                    - generic [ref=e544]:
                      - button [ref=e548] [cursor=pointer]: Enable table equivalent for screen reader users
                      - list [ref=e549]
                  - generic [ref=e552]:
                    - link [ref=e553] [cursor=pointer]:
                      - /url: javascript:void(0);
                      - text: View Report
                    - generic [ref=e554]:
                      - generic [ref=e555]: As of Yesterday at 3:53 PM
                      - button [ref=e556] [cursor=pointer]:
                        - generic [ref=e557]: Refresh
                        - img [ref=e561]
              - article [ref=e565]:
                - article [ref=e567]:
                  - generic [ref=e568]:
                    - heading [level=2] [ref=e571]:
                      - link [ref=e572] [cursor=pointer]:
                        - /url: javascript:void(0);
                        - generic [ref=e573]: My Joint Business Plans
                    - link [ref=e575] [cursor=pointer]:
                      - /url: javascript:void(0);
                      - text: More
                  - generic [ref=e578]:
                    - link [ref=e584] [cursor=pointer]:
                      - /url: javascript:void(0);
                      - generic [ref=e585]: List Views
                      - heading [level=2] [ref=e586]:
                        - generic [ref=e587]: My Joint Business Plans
                        - generic [ref=e588]: 0 items, sorted by Joint Business Planning Initiative Name
                      - img [ref=e593]
                    - generic [ref=e597]:
                      - text: There's nothing in My Joint Business Plans yet.
                      - paragraph [ref=e598]: When records are added to this list view, you'll see them here.
              - article [ref=e601]:
                - heading [level=2] [ref=e605]:
                  - generic [ref=e606]: Recently-Viewed Opportunities (0)
            - generic [ref=e607]:
              - paragraph [ref=e611]:
                - strong [ref=e612]: Build Your Pipeline
              - article [ref=e614]:
                - heading [level=2] [ref=e618]:
                  - generic [ref=e619]: Leads by Source
                - generic [ref=e621]:
                  - generic [ref=e624]:
                    - text: Loading report chart...
                    - generic [ref=e626]:
                      - button [ref=e630] [cursor=pointer]: Enable table equivalent for screen reader users
                      - list [ref=e631]
                  - generic [ref=e634]:
                    - link [ref=e635] [cursor=pointer]:
                      - /url: javascript:void(0);
                      - text: View Report
                    - generic [ref=e636]:
                      - generic [ref=e637]: As of Yesterday at 3:53 PM
                      - button [ref=e638] [cursor=pointer]:
                        - generic [ref=e639]: Refresh
                        - img [ref=e643]
              - article [ref=e647]:
                - article [ref=e649]:
                  - generic [ref=e650]:
                    - heading [level=2] [ref=e653]:
                      - link [ref=e654] [cursor=pointer]:
                        - /url: javascript:void(0);
                        - generic [ref=e655]: My Leads
                    - link [ref=e657] [cursor=pointer]:
                      - /url: javascript:void(0);
                      - text: More
                  - generic [ref=e660]:
                    - link [ref=e666] [cursor=pointer]:
                      - /url: javascript:void(0);
                      - generic [ref=e667]: List Views
                      - heading [level=2] [ref=e668]:
                        - generic [ref=e669]: My Leads
                        - generic [ref=e670]: 0 items, sorted by Name
                      - img [ref=e675]
                    - generic [ref=e679]:
                      - text: There's nothing in My Leads yet.
                      - paragraph [ref=e680]: When records are added to this list view, you'll see them here.
              - article [ref=e683]:
                - heading [level=2] [ref=e687]:
                  - generic [ref=e688]: Recently-Viewed Leads (0)
    - dialog "Your session has ended" [ref=e693]:
      - generic [ref=e694]:
        - heading "Your session has ended" [active] [level=1] [ref=e696]
        - generic [ref=e697]:
          - text: We're sorry to report that something has happened to your session. Log in again, or return to the page to copy any unsaved text.
          - status [ref=e698]
        - generic [ref=e699]:
          - button "Return to Page" [ref=e700] [cursor=pointer]
          - button "Log In" [ref=e701] [cursor=pointer]
  - generic:
    - status
```